...
export function AllStudiesDetailPage({ submissionId }: { submissionId: number }) {
  const selectedSubmission = useMemo(() => {
    return mockReviewSubmissions.find((item) => item.id === submissionId) ?? mockReviewSubmissions[0];
  }, [submissionId]);
